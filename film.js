...
plakat.heightt = film.plakat.vyska;

// 6 ukol
const premieraHtml = document.querySelector('#premiera');
const premiera = dayjs(film.premiera);

premieraHtml.innerHTML = '';
premieraHtml.innerHTML = `Premiéra <strong>${premiera.format(
  'D. M. YYYY',
)}</strong>`;
console.log('premiera.format', premiera.format('D. M. YYYY'));

const dnes = dayjs();
const rozdilMeziDnami = premiera.diff(dnes, 'days');
...
let odhvezdickovano = 0;

const hodnoceniHvezda = (pocetHvezd) => {
...
const hvezdaMouseEnter = (event) => {
  hodnoceniHvezda(Number(event.target.textContent));
};

const hvezdaMouseLeave = () => {
  hodnoceniHvezda(odhvezdickovano);
};

const hvezdaMouseSelected = (event) => {
  odhvezdickovano = Number(event.target.textContent);
  hodnoceniHvezda(odhvezdickovano);
};
...
hvezdy.forEach((hvezda) => {
  ...
  hvezda.addEventListener('click', hvezdaMouseSelected);
});
